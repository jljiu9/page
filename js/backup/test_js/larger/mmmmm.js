...
let org_list = []
let larger_list = []
let path_one = 'js\\test\\1'
let path_two = 'js\\test\\2'
...
let loadsmt = async (browser) => {
    ...
    xiu.f.map(x=>{  // 过滤目的目录下的文件
        let name = x.split('\\')
        list.f.match(name[name.length-2]+'\\'+path.basename(x).split('.')[0])
    })
...
let imglarger = async (browser,list) => {
    ...
    await page.evaluate(async()=>{
        ...
        let btn_xx = setInterval(btn_X,3000)
